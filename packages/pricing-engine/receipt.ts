...
export interface CartItem {
  ...
  price: number;
  qty: number;
  /** Display name for line items not present in the catalog (e.g. loyalty rewards). */
  name?: string;
  /** Display category for line items not present in the catalog. */
  category?: string;
}
 ...
 *
 * Not every SKU is a catalog product — loyalty rewards and other synthetic
 * line items are injected at checkout and carry their own display metadata.
 * For those, fall back to the item's own name/category (and finally the SKU)
 * instead of dereferencing a missing catalog entry.
 */
